...
public:
	...
	ventas(string cant, string unit, int pr, int vn, int nf, string se, string fft, int idc, int ide, string ing): venta(vn,nf,cant,unit,se,fft,idc,ide,ing,pr) {
	...
	void setid_Ventas(int vn) { id_Ventas = vn; }
	...
	int setid_Ventas() { return id_Ventas; }
	...
	void crearVentaDetalle() {
		...
		if (cn.getConectar()) {
			...
			string insert = " insert into ventas_detalle (id_Ventas,id_Productos, cantidad, precio_unitario) VALUES ('" + vent+ "','" + produc + "',  '" + cantidad + "','" + precio_uni + "' );";
			// executar el query
			const char* h = insert.c_str();
			q_estado = mysql_query(cn.getConectar(), h);
			if (!q_estado) {
